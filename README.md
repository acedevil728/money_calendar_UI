...
|---|---|---|---|---|
| V1.2 - backend 리팩토링 및 단위 테스트 확장 | 2026-02-22 |  | 불필요/중복 코드 제거, 가독성 개선, 모델 소스 통합, UT 보강 | `main.py`/`crud.py` 중복 로직 헬퍼화, 미사용 코드 제거, `models.py`를 `models_core` 재노출 레이어로 정리, `backend/test` 테스트/가이드 확장 |
